...
const ScorekeeperModal: React.FC<ScorekeeperModalProps> = ({ booking, onClose, onUpdateScore, onFinalizeMatch, currentTime }) => {
    ...
    const matchStartTime = new Date(booking.date);
    const [hours, minutes] = booking.time.split(':').map(Number);
    matchStartTime.setHours(hours, minutes, 0, 0);

    const matchEndTime = new Date(matchStartTime.getTime() + 60 * 60 * 1000); // 60 minutos después del inicio
    const canFinalize = currentTime.getTime() >= matchEndTime.getTime();
    ...
    return (
        <>
                ...
                <div className="bg-gray-800 text-white rounded-2xl shadow-xl w-full max-w-md m-4" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="p-4 bg-gray-900/50 border-t border-white/10 flex flex-col sm:flex-row justify-end gap-3">
                        ...
                        <button 
                            ...
                            title={!canFinalize ? "Solo se puede finalizar el partido una vez terminada la hora de juego." : ""}
                            className="py-2 px-5 rounded-lg font-semibold bg-red-600 text-white hover:bg-red-700 shadow-sm disabled:bg-gray-500 disabled:cursor-not-allowed"
                        >
                            Finalizar Partido
                        </button>
                    </div>
            ...
            <ConfirmationModal
                ...
                message={`El marcador final será ${teamNameA} ${scoreA} - ${scoreB} ${teamNameB}. Esta acción marcará el partido como completado y no se podrá modificar. ¿Estás seguro?`}
