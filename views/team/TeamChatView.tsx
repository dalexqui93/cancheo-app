...
const TeamChatView: React.FC<TeamChatViewProps> = ({ team, currentUser, onBack, onUpdateTeam, addNotification }) => {
    ...
    return (
        <div className="fixed inset-0 h-screen relative text-white animate-fade-in team-chat-bg">
            <div className="absolute inset-0 bg-black/60 z-0"></div>
             {/* Header */}
            <header className="absolute top-0 left-0 right-0 h-20 z-20 flex items-center p-4 border-b border-white/10 bg-black/20 backdrop-blur-sm">
            ...
            {/* Messages */}
            <main className="absolute top-20 bottom-24 left-0 right-0 overflow-y-auto p-4 z-10">
                {isLoading ? (
            ...
            {/* Input */}
            <footer className="absolute bottom-0 left-0 right-0 h-24 z-20 p-4 border-t border-white/10 bg-black/20 backdrop-blur-sm">
                {canSendMessage ? (
                    <>
                        {attachment && (
